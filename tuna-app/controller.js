...
return{
	get_all_prefixes: function(req, res){
		console.log("getting all tuna from database: ");

		var array = req.params.prefixes.split("-");
		console.log(array);
		var startKey = array[0].replace('@', '/');
		var endKey = array[1].replace('@', '/');
	...
	allocate_prefix: function(req, res){
		console.log("submit recording of a tuna catch: ");

		var array = req.params.prefix.split("-");
		console.log(array);

		var IPv6_address = array[0].replace('@', '/')
		var AS_number = array[1]
	...
	change_holder: function(req, res){
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    if (isProposalGood) {
		        ...
		        console.error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
		        res.send("Error: no prefix found");
		        // throw new Error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
		    }
		}).then((results) => {
		    console.log('Send transaction promise and event listener promise have completed');
